          ...
          {navGroups.map(group => (
            <div key={group.label}>
              ...
              {group.items.map(item => (
                <button key={item.id} onClick={() => item.id === 'brief' ? router.push('/portal/studio/brief') : item.id === 'pitches' ? router.push('/portal/studio/pitches') : item.id === 'projects' ? router.push('/portal/studio/projects') : setActiveView(item.id)} style={{ display: 'flex', alignItems: 'center', width: '100%', padding: '8px 10px', borderRadius: 5, fontSize: 12, color: activeView === item.id ? '#C8C2BB' : 'rgba(200,194,187,0.38)', background: activeView === item.id ? 'rgba(61,71,86,0.4)' : 'transparent', border: activeView === item.id ? '0.5px solid rgba(200,194,187,0.09)' : '0.5px solid transparent', cursor: 'pointer', marginBottom: 1, textAlign: 'left', fontFamily: 'inherit' }}>
                  {item.label}
