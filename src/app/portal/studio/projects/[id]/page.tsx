...
export default function ProjectDetailPage() {
  ...
  async function loadProject() {
    ...
    if (!error && data) {
      setProject(data)
      try {
        const stored = localStorage.getItem(`deliverables_${id}`)
        if (stored) setDeliverables(JSON.parse(stored))
      } catch {}
    }
    setLoading(false)
  }
  ...
  function toggleDeliverable(did: string) {
    const updated = deliverables.map(d => d.id === did ? { ...d, done: !d.done } : d)
    setDeliverables(updated)
    localStorage.setItem(`deliverables_${id}`, JSON.stringify(updated))
  }

  function addDeliverable() {
    if (!newDeliverable.trim()) return
    const updated = [...deliverables, { id: Date.now().toString(), name: newDeliverable.trim(), done: false }]
    setDeliverables(updated)
    localStorage.setItem(`deliverables_${id}`, JSON.stringify(updated))
    setNewDeliverable('')
  }
  ...
  function removeDeliverable(did: string) {
    const updated = deliverables.filter(d => d.id !== did)
    setDeliverables(updated)
    localStorage.setItem(`deliverables_${id}`, JSON.stringify(updated))
  }
  ...
  if (!project) return (
    <main style={{ background: '#0E1014', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Inter, sans-serif' }}>
      <div style={{ color: 'rgba(200,194,187,0.4)', fontSize: 13 }}>Project not found. <button onClick={() => router.push('/portal/studio/projects')} style={{ color: '#C8C2BB', background: 'transparent', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>← Back</button></div>
    </main>
  )

  const stageIdx = STAGES.indexOf(project.stage)
  const completedCount = deliverables.filter(d => d.done).length
  const completedProgress = deliverables.length > 0 ? Math.round((completedCount / deliverables.length) * 100) : project.progress
  ...
  return (
    <main style={{ background: '#0E1014', minHeight: '100vh', fontFamily: 'Inter, sans-serif', color: '#C8C2BB', fontSize: 13 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', height: 'calc(100vh - 171px)' }}>
        ...
        <div style={{ overflowY: 'auto', padding: 28 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              <div style={{ background: '#1A1F28', border: '0.5px solid rgba(200,194,187,0.09)', borderRadius: 7, overflow: 'hidden' }}>
                ...
                <div style={{ padding: 18, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  ...
                  <div><label style={lbl}>Contact</label><input style={inp} value={project.contact} onChange={e => updateField('contact', e.target.value)} /></div>
                  ...
                  {project.category === 'Property' && (
                    <div style={{ gridColumn: 'span 2' }}><label style={lbl}>Property address</label><input style={inp} value={project.address} onChange={e => updateField('address', e.target.value)} /></div>
                  )}
                </div>
              </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 14 }}>
                    {deliverables.map(d => (
                      <div key={d.id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        <button onClick={() => removeDeliverable(d.id)} style={{ fontSize: 13, color: 'rgba(210,90,90,0.5)', background: 'transparent', border: 'none', cursor: 'pointer', flexShrink: 0, padding: '4px' }}>✕</button>
                      </div>
                    ))}
                    {deliverables.length === 0 && <div style={{ fontSize: 12, color: 'rgba(200,194,187,0.25)', padding: '8px 0' }}>No deliverables yet — add one below</div>}
                  </div>
                  <div style={{ display: 'flex', gap: 10 }}>
          ...
          {activeTab === 'notes' && (
            <div style={{ background: '#1A1F28', border: '0.5px solid rgba(200,194,187,0.09)', borderRadius: 7, overflow: 'hidden' }}>
              ...
              <div style={{ padding: 18 }}>
                <textarea value={project.general_notes} onChange={e => updateField('general_notes', e.target.value)} style={{ ...inp, resize: 'vertical' as const, lineHeight: 1.75, minHeight: 420, fontSize: 13 }} placeholder="Project notes, client feedback, shoot day observations, anything relevant..." />
              </div>
            </div>
          )}
          ...
          {activeTab === 'editor' && (
            <div style={{ background: '#1A1F28', border: '0.5px solid rgba(200,194,187,0.09)', borderRadius: 7, overflow: 'hidden' }}>
              ...
              <div style={{ padding: 18 }}>
                ...
                <textarea value={project.editor_notes} onChange={e => updateField('editor_notes', e.target.value)} style={{ ...inp, resize: 'vertical' as const, lineHeight: 1.75, minHeight: 380, fontSize: 13 }} placeholder="e.g. Grade — warm, filmic. Music — cinematic, no lyrics. Feature kitchen prominently. Client wants 60 sec version for Instagram..." />
              </div>
            </div>
          )}
          ...
          {activeTab === 'brief' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ background: '#1A1F28', border: '0.5px solid rgba(200,194,187,0.09)', borderRadius: 7, overflow: 'hidden' }}>
                ...
                <div style={{ padding: 18 }}>
                  {!project.address && <div style={{ fontSize: 12, color: 'rgba(200,194,187,0.35)', marginBottom: 12 }}>Set a property address in Overview first.</div>}
                  {!brief ? (
                    ...
                    <div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 16 }}>
                        {[['Rateable value', brief.property?.rateableValue], ['Last sale price', brief.property?.lastSalePrice], ['Sale year', brief.property?.lastSaleDate]].map(([label, value]) => (
                      ...
                      {brief.mapboxImageUrl && <div style={{ borderRadius: 6, overflow: 'hidden', marginBottom: 16, border: '0.5px solid rgba(200,194,187,0.09)' }}><img src={brief.mapboxImageUrl} alt="Satellite view" style={{ width: '100%', display: 'block' }} /></div>}
                      {brief.weather && (
        ...
        <div style={{ background: '#0F1216', borderLeft: '0.5px solid rgba(200,194,187,0.09)', overflowY: 'auto' }}>
          ...
          <div style={{ padding: '14px 16px', borderBottom: '0.5px solid rgba(200,194,187,0.09)' }}>
            ...
            {project.contact && <div style={{ fontSize: 11, color: 'rgba(200,194,187,0.4)', marginBottom: 2 }}>{project.contact}</div>}
          ...
          <div style={{ padding: '14px 16px', borderBottom: '0.5px solid rgba(200,194,187,0.09)' }}>
            ...
            {project.drive_url ? (
              <a href={project.drive_url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, color: 'rgba(100,150,220,0.8)', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 6 }}>📁 Open project folder →</a>
            ) : (
