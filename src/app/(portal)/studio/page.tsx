      ...
      <div style={{ padding: 28 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 24 }}>
          {[
            ...
            <a key={label} href={href} style={{ background: '#1A1F28', border: '0.5px solid rgba(200,194,187,0.09)', borderRadius: 7, padding: '18px 20px', textDecoration: 'none', display: 'block', transition: 'border-color 0.15s' }}
              onMouseEnter={e => (e.currentTarget.style.borderColor = 'rgba(200,194,187,0.2)')}
              onMouseLeave={e => (e.currentTarget.style.borderColor = 'rgba(200,194,187,0.09)')}>
        ...
        <div style={{ background: 'rgba(100,200,130,0.06)', border: '0.5px solid rgba(100,200,130,0.2)', borderRadius: 7, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: 'rgba(200,194,187,0.38)' }}>Supabase is live · All tables created · Row level security enabled · Logged in as {user?.email}</div>
          </div>
        </div>
